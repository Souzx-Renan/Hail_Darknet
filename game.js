...
const BOSSES = [
  ...
  {
    id: "boss3",
    name: "Bill Gates - Primeira Forma",
    art: "imgs/billgates.png",
    shield: 8,
    health: 50,
    maxHealth: 50,
    attackRange: [5, 9],
  ...
  {
    id: "boss4",
    name: "Bill Gates - Forma Final",
    art: "imgs/billgates.png",
    shield: 12,
    health: 80,
    maxHealth: 80,
    attackRange: [8, 15],
    ability: "Dominação Total: Controla uma carta inimiga por 1 turno + Regeneração",
  ...
  static createHeroElement(hero, options = {}) {
    ...
    heroEl.innerHTML = `
      ...
      <div class="hero-art">
        ${
          isImage
            ? `<img src="${hero.art}" alt="${hero.name}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
           <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size:3rem;">${hero.name.charAt(0)}</div>`
            : `<div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:3rem;">${hero.art || hero.name.charAt(0)}</div>`
        }
      </div>
      <div class="hero-stats">
        <div class="hero-stat stat-attack">⚔️ ${minAttack}-${maxAttack}</div>
        <div class="hero-stat stat-shield">🛡️ ${hero.shield}</div>
        <div class="hero-stat stat-health">❤️ ${hero.health}/${hero.maxHealth}</div>
      </div>
    `
...
class BattleManager {
  static initBattle(gameState) {
    ...
    this.addBattleLog(`Fase ${gameState.currentPhase}: Duelo contra ${boss.name} iniciado!`)
    NotificationSystem.show(`Enfrentando ${boss.name}!`, "warning")
  }

  static drawHeroes(gameState, count = 1) {
    for (let i = 0; i < count; i++) {
      if (gameState.playerSquad.length > 0 && gameState.playerHand.length < GAME_CONFIG.MAX_HAND_SIZE) {
  ...
  static attack(gameState, attackerIndex) {
    ...
    if (attacker.ability.includes("Precisão")) {
      finalDamage = Math.max(damage - Math.max(0, gameState.currentBoss.shield - 2), damage)
      this.addBattleLog(`${attacker.name} usa Precisão, ignorando parte do escudo!`, "ability")
    }

    gameState.currentBoss.health -= finalDamage

    this.addBattleLog(`${attacker.name} ataca causando ${finalDamage} de dano!`, "damage")
  ...
  static applyHeroAbilities(gameState, hero, baseDamage) {
    ...
    if (hero.ability.includes("Cura")) {
      const woundedAllies = gameState.playerField.filter((h) => h && h.health < h.maxHealth)
      if (woundedAllies.length > 0) {
        ...
        this.addBattleLog(`${hero.name} cura ${allyToHeal.name} em ${healAmount} pontos!`, "heal")
      }
    }
    ...
    if (hero.ability.includes("Porrada") && hero.health === hero.maxHealth) {
      damage += 3
      this.addBattleLog(`${hero.name} desfere uma Porrada devastadora! (+3 dano)`, "ability")
    }

    // Crítico (Messias)
    if (hero.ability.includes("Crítico") && Math.random() < 0.3) {
      damage = hero.attackRange[1] // Dano máximo
      this.addBattleLog(`${hero.name} acerta um CRÍTICO! Dano máximo!`, "ability")
    }

    // Golpe Sagrado (Cidão)
    if (hero.ability.includes("Golpe Sagrado") && Math.random() < 0.4) {
      damage += 2
      this.addBattleLog(`${hero.name} canaliza poder divino! (+2 dano)`, "ability")
    }

    // Magia Elemental (Camila)
    if (hero.ability.includes("Magia Elemental") && Math.random() < 0.2) {
      damage += 4
      this.addBattleLog(`${hero.name} libera poder elemental! (+4 dano)`, "ability")
    }

    // Sorte (Erinaldo) - já é aleatório por natureza
    if (hero.name === "Erinaldo") {
      gameState.battleStats.maxErinaldoDamage = Math.max(gameState.battleStats.maxErinaldoDamage, damage)
      this.addBattleLog(`${hero.name}: "Vai que é tua!" (${damage} de dano)`, "ability")
    }
  ...
  static bossTurn(gameState) {
    ...
    if (gameState.currentBoss.ability.includes("Ataque Bugado")) {
      ...
      this.addBattleLog(`${gameState.currentBoss.name} executa ATAQUE BUGADO!`, "ability")
    } else {
    ...
    this.addBattleLog(`${gameState.currentBoss.name} ataca ${target.name} causando ${damage} de dano!`, "damage")
  ...
  static applyBossAbilities(gameState) {
    ...
    if (boss.ability.includes("Ataque Bugado")) {
      ...
      this.addBattleLog(`${boss.name}: "SISTEMA INSTÁVEL... CALCULANDO DANO..."`, "ability")
    }
    ...
    if (boss.ability.includes("Drenar Vida")) {
      ...
      this.addBattleLog(`${boss.name} drena energia vital e se regenera!`, "heal")
    }

    // Controle de Sistema (Bill Gates - Primeira Forma)
    if (boss.ability.includes("Controle de Sistema") && Math.random() < 0.3) {
      this.addBattleLog(`${boss.name} hackeia o sistema! Seu próximo turno será pulado!`, "ability")
    ...
    if (boss.ability.includes("Dominação Total")) {
      ...
      if (controlledHeroes.length > 0) {
        const controlledHero = controlledHeroes[Math.floor(Math.random() * controlledHeroes.length)]
        ...
        gameState.playerField.forEach((hero, index) => {
          if (hero && hero !== controlledHero) {
            ...
            if (hero.health <= 0) {
              ...
              this.addBattleLog(`${hero.name} foi destruído pela dominação digital!`, "damage")
            }
          }
        })

        this.addBattleLog(`${boss.name} controla ${controlledHero.name} contra seus aliados!`, "ability")
      }
    }
  }
  ...
  static handleVictory(gameState) {
    ...
    const allHeroesHealthy = gameState.playerField.filter((h) => h).every((hero) => hero.health === hero.maxHealth)
    if (allHeroesHealthy) {
      gameState.battleStats.perfectWins++
    }

    this.addBattleLog(`${gameState.currentBoss.name} foi derrotado! VITÓRIA!`, "heal")

    // Verificar se é a primeira forma do Bill Gates
    if (gameState.currentBoss.isFirstForm) {
      this.addBattleLog(`"IMPOSSÍVEL! Mas... eu tenho uma FORMA FINAL!"`, "ability")
      NotificationSystem.show("Bill Gates se transforma! Prepare-se para a batalha final!", "warning", 6000)
    ...
    if (gameState.currentPhase >= GAME_CONFIG.PHASES) {
      ...
      this.addBattleLog(`"NÃÃÃOOO! O SISTEMA... ESTÁ... FALHANDO..."`, "ability")
      NotificationSystem.show("🎉 VITÓRIA ÉPICA! Você salvou Eldrath do Senhor das Trevas Digitais!", "success", 10000)
      ...
      NotificationSystem.show(
        `Fase ${gameState.currentPhase - 1} concluída! Preparando próximo desafio...`,
        "success",
        5000,
      )
...
class AchievementSystem {
  ...
  static unlockAchievement(id) {
    ...
    if (achievement && !achievement.unlocked) {
      ...
      NotificationSystem.show(`🏆 Conquista desbloqueada: ${achievement.name}!`, "achievement", 6000)
    }
  }
...
class AudioManager {
  ...
  static switchToBattleMusic() {
    ...
    this.battleMusic.play().catch((e) => console.log("Erro ao tocar música:", e))
  }
}
...
class UI {
  ...
  static setupEventListeners() {
    ...
    document.getElementById("drawCardBtn").addEventListener("click", () => {
      if (game.energy > 0 && game.playerSquad.length > 0) {
        ...
        NotificationSystem.show("Energia insuficiente ou sem heróis na reserva!", "warning")
      }
    })
    ...
    document.getElementById("menuBtn").addEventListener("click", () => {
      if (confirm("Deseja voltar ao menu principal? O progresso da batalha será perdido.")) {
        AudioManager.switchToMenuMusic()
  ...
  static showScreen(screenId) {
    ...
    const screens = ["storySlideshow", "loadingScreen", "mainMenu", "gameScreen", "squadSelection"]
  ...
  static toggleHeroSelection(hero, element) {
    ...
    if (isSelected) {
      ...
      if (game.playerSquad.length < GAME_CONFIG.SQUAD_SIZE) {
        ...
        NotificationSystem.show(`Máximo de ${GAME_CONFIG.SQUAD_SIZE} heróis!`, "warning")
      }
    }
  ...
  static updateSquadCounter() {
    ...
    if (startBattleBtn) {
      startBattleBtn.disabled = game.playerSquad.length !== GAME_CONFIG.SQUAD_SIZE
    }
  }
  ...
  static generateRandomSquad() {
    ...
    game.playerSquad = shuffled.slice(0, GAME_CONFIG.SQUAD_SIZE).map((hero) => ({ ...hero }))
  ...
  static updateBattleUI(gameState) {
    ...
    if (manaValueEl) manaValueEl.textContent = `${gameState.mana}/${gameState.maxMana}`
    if (energyValueEl) energyValueEl.textContent = `${gameState.energy}/${gameState.maxEnergy}`
    ...
    if (gameState.currentBoss) {
      ...
      if (enemyHealthFillEl) {
        const healthPercent = (gameState.currentBoss.health / gameState.currentBoss.maxHealth) * 100
      ...
      if (enemySlot) {
        ...
        const isImage = gameState.currentBoss.art && gameState.currentBoss.art.match(/\.(png|jpg|jpeg|gif)$/i)
        ...
        bossCard.innerHTML = `
          ...
          <div class="hero-art">
            ${
              isImage
                ? `<img src="${gameState.currentBoss.art}" alt="${gameState.currentBoss.name}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
               <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size:3rem;">${gameState.currentBoss.name.charAt(0)}</div>`
                : `<div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:3rem;">${gameState.currentBoss.art || gameState.currentBoss.name.charAt(0)}</div>`
            }
          </div>
          <div class="hero-stats">
            <div class="hero-stat stat-attack">⚔️ ${gameState.currentBoss.attackRange[0]}-${gameState.currentBoss.attackRange[1]}</div>
            <div class="hero-stat stat-shield">🛡️ ${gameState.currentBoss.shield}</div>
            <div class="hero-stat stat-health">❤️ ${gameState.currentBoss.health}</div>
          </div>
        `
    ...
    if (drawCardBtn) {
      drawCardBtn.disabled = gameState.energy <= 0 || !gameState.isPlayerTurn || gameState.playerSquad.length === 0
    }
  }
    ...
    gameState.playerHand.forEach((hero, index) => {
      const heroEl = HeroManager.createHeroElement(hero, {
        onClick: (hero, element) => {
          ...
          document.querySelectorAll(".hand-cards .hero-card").forEach((c) => c.classList.remove("selected"))

          // Selecionar este herói
          element.classList.add("selected")
          gameState.selectedHandCard = index

          NotificationSystem.show(`${hero.name} selecionado. Clique em um slot vazio para invocar.`, "info")
        },
      })
  ...
  static showHeroModal(hero) {
    ...
    if (modalHeroDescription) modalHeroDescription.textContent = hero.description
